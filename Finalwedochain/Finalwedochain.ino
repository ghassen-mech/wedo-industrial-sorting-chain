...
void setup() {
  Serial.begin(9600);
//IR pins configuration as input
  pinMode(IR1, INPUT);
  pinMode(IR2, INPUT);
  pinMode(IR3, INPUT);
  pinMode(IR4, INPUT);
...
Serial.print(" red  ");Serial.println(red);
  ...
  if (ir2 == 0 && state2 && security2 && red == 1 )
  {
    ...
    //adf
     // pieces++;
      //sending data to adafruit platform through MQTT protocol
      if( pieces>2 ){
            io.run();
...
//Receiving data from adfruit(commands wish sent from the mobile App "start/stop") 
